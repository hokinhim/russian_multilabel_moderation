...
Полученные результаты показывают, что RuBERT-классификатор превосходит TF-IDF baseline по всем основным метрикам. Transformer-модель лучше решает задачу как на уровне отдельных меток, так и на уровне полного набора меток для каждого текста. Доля ошибочных решений по парам «текст-класс» стала меньше.

Рост recall по редким классам (с 0.705 у TF-IDF до 0.849 у RuBERT) важен для задачи модерации, поскольку низкий recall означает, что модель пропускает потенциально опасные сообщения. RuBERT лучше обнаруживает не только частотные и лексически очевидные категории, но и более сложные темы, где важен контекст.

Поклассовый анализ F1 показывает, что RuBERT превосходит TF-IDF по всем категориям. Наиболее высокие значения F1 достигнуты для классов religion, drugs, gambling, suicide, weapons и terrorism. Это говорит о том, что данные темы имеют достаточно устойчивые языковые признаки, которые хорошо распознаются обеими моделями, но RuBERT извлекает их более эффективно. Наиболее сложными остаются классы social_injustice, offline_crime, politics и pornography. Это может быть связано с большей тематической неоднозначностью таких текстов.
